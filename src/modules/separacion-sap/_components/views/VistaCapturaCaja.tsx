...
import { actualizarEstadoCajaSeparacionSap, registrarCapturaSeparacionSap } from '../../infrastructure/cajas/cajas-api';
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-white text-slate-700 border border-slate-200 shadow-xs hover:bg-slate-50"
          >
            <Tag className="w-3.5 h-3.5 text-teal-700" /> Ver Detalle
          </button>
          <button
            type="button"
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-black font-mono text-slate-900">{cajaActiva.numeroCaja}</h1>
            <CentroBadge centro={cajaActiva.centro} />
            <EstadoBadge estado={cajaActiva.estado} />
          </div>
          <div className="text-xs font-mono text-slate-500">
            {llenos.length} Sub-Grupo{llenos.length !== 1 ? 's' : ''} lleno{llenos.length !== 1 ? 's' : ''} ·{' '}
            {totales.cantidadCapturada} uds capturadas
          </div>
        </div>
            ...
            <table className="w-full text-xs text-left">
              <thead className="text-slate-500 font-semibold border-b border-slate-100">
                <tr>
                  ...
                  <th className="py-2 pr-2">Valoración</th>
                  <th className="py-2 pr-2 text-center">Progreso</th>
                  ...
                  return (
                    <tr key={sg.id} className={activo ? 'bg-teal-50/60' : ''}>
                      <td className="py-2 pr-2">
                        ...
                        <div className="text-[10px] text-slate-500 truncate max-w-[200px]">{sg.materialTexto}</div>
                      </td>
                      <td className="py-2 pr-2">{TIPOS_PRODUCTO[sg.tipoProducto]?.label}</td>
                      <td className="py-2 pr-2">
                        <ValoracionBadge valoracion={sg.valoracion} />
                      </td>
                      <td className="py-2 pr-2 text-center font-mono font-bold">
      ...
      {modo === 'lista' && abierta && (
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 bg-teal-700 text-white text-xs font-bold rounded-lg hover:bg-teal-800 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <Plus className="w-4 h-4" /> Crear Sub-Grupo
          </button>
          {hayEnProgreso && subgrupoActivo && (
            <button
              type="button"
              onClick={() => setModo('pistoleando')}
              className="flex items-center gap-2 px-5 py-3 bg-sky-700 text-white text-xs font-bold rounded-lg hover:bg-sky-800"
            >
              <Barcode className="w-4 h-4" /> Continuar pistoleo ({progresoSubgrupo}/{metaSubgrupo})
            </button>
          )}
          <button
            type="button"
            ...
            className={`flex items-center gap-2 px-5 py-3 text-xs font-bold rounded-lg ${
              puedeTerminar
                ? 'bg-amber-600 text-white hover:bg-amber-700'
                : 'bg-slate-100 text-slate-400 cursor-not-allowed'
            }`}
          >
            <Lock className="w-4 h-4" /> Terminar Caja
          </button>
        </div>
      )}

      {modo === 'lista' && !puedeTerminar && llenos.length === 0 && abierta && sapCargado && (
        <p className="text-xs text-slate-500">Complete al menos un Sub-Grupo para habilitar &quot;Terminar Caja&quot;.</p>
      )}
